...
class MealDetailVC: UIViewController {
    ...
    private func configureMealDetails(meal: Meal) {
        ...
        ingredientsNumberLabel.text = "\(meal.ingredientsNumber) ingredients"
        ...
        let updatedFeatures = [
            (iconName: "clock", text: "\(meal.time)"),
            (iconName: "ingreds", text: "\(meal.ingredientsNumber) ingredients"),
            (iconName: "chef", text: meal.level)
        ]

        let newFeatureContainer = createFeatureContainer(features: updatedFeatures)
        
        if let oldFeatureContainer = contentView.subviews.first(where: { $0 is UIStackView }) {
            oldFeatureContainer.removeFromSuperview()
        }
        
        contentView.addSubview(newFeatureContainer)
        
        NSLayoutConstraint.activate([
            newFeatureContainer.centerXAnchor.constraint(equalTo: contentView.centerXAnchor),
            newFeatureContainer.topAnchor.constraint(equalTo: nameLabel.bottomAnchor, constant: 20),
            newFeatureContainer.leadingAnchor.constraint(greaterThanOrEqualTo: contentView.leadingAnchor, constant: 20),
            newFeatureContainer.trailingAnchor.constraint(lessThanOrEqualTo: contentView.trailingAnchor, constant: -20)
        ])
    }
